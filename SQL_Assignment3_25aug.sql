...
--3)Write a query that will produce the snum values of all salespeople from the Orders table (with theduplicate values suppressed)
select distinct snum from orders

--4) Write a query that will give you all orders for more than Rs. 1,000
select * from orders where amt>1000

--5) Write a query that will give you the names and cities of all salespeople in London with acommission above 0.10
...
select * from customers where rating>100 or city like 'Rome'

--7)What will be the output from the following query? Select * from Orderswhere (amt < 1000 ORNOT (odate = ‘1990-10-03’AND cnum > 2003));
Select * from Orders where (amt < 1000 OR NOT (odate = '1990-10-03'AND cnum > 2003));

--8)What will be the output of the following query?Select * from Orderswhere NOT ((odate = ‘1990-10-03’ OR snum >1006) AND amt >= 1500)
