...
Select snum,sum(amt) as total_amt from orders group by snum having(sum(amt)>(select max(amt) from orders))
-- 4)Write a query that selects all customers whose ratings are equal to or greater than ANY of Serres.
 select cname ,rating from customers where rating>=(select max(rating) max_rating from customers where snum=(select snum from salespeople where sname='serres'))

-- 5)Write a query using ANY or ALL that will find all salespeople who have no customers located in
-- their city.
--Select sname,snum  from salespeople where (snum ,city)not in(select snum,city from customers )
